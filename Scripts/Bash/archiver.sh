            ...
            case "$ext" in
                ...
                jpg|jpeg|png|gif|bmp|tiff|tif|webp|dng|raw|cr2|nef|arw|orf|sr2|raf|rw2|pef|srw)                
                    if is_image_file "$file"; then
                        if [[ "$ext" == "png" ]]; then
                            ...
                            magick -debug Exception "$file" -quality 75 "$out_file"
                        else
                ...
                mp4|mov|avi|mkv|webm|flv|wmv|m4v|3gp|mpeg|mpg)
                    if is_video_file "$file"; then
                        ...
                        echo "Output to: $out_file"
                        ffmpeg -loglevel error -y -i "$file" -c:v libx265 -preset slow -crf 28 -c:a aac -b:a 128k "$out_file"
                        processed=$((processed+1))
                ...
                *)
                    ...
                    if is_image_file "$file"; then
                        ...
                        echo "Output to: $out_file"
                        ffmpeg -loglevel error -i -y "$file" -c:v libx265 -preset slow -crf 28 -c:a aac -b:a 128k "$out_file"
                        processed=$((processed+1))
                    else
...
echo "Compressed archive size: $(du -sh "$ZIP_NAME" | cut -f1)"
